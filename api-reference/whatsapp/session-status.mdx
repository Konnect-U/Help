...
```bash cURL
curl -X GET "https://app.konnect-u.com//api/user/whatsapp/session-status?sender_id=12&recipient_phone=+1234567890" \
...
const response = await fetch(
  `https://app.konnect-u.com//api/user/whatsapp/session-status?${params}`,
  {
    headers: {
...
response = requests.get(
    'https://app.konnect-u.com//api/user/whatsapp/session-status',
