...
```bash cURL
curl -X GET "https://app.konnect-u.com//api/user/whatsapp/senders/12/templates" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

```bash All templates (including pending/rejected)
curl -X GET "https://app.konnect-u.com//api/user/whatsapp/senders/12/templates?status=all" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
const response = await fetch(
  `https://app.konnect-u.com//api/user/whatsapp/senders/${senderId}/templates`,
  {
    headers: {
...
response = requests.get(
    f'https://app.konnect-u.com//api/user/whatsapp/senders/{sender_id}/templates',
